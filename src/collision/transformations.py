...
def nearest_neighbor(key, query, k=1):
    ...
    if k == 1:
        dist, idx = torch.where(sqr_dist > 1e-7, torch.sqrt(sqr_dist + 1e-8), torch.zeros_like(sqr_dist)).min(dim=1)
    else: # Here the torch.sqrt(sqr_dist + 1e-8) has been added
        dist, idx = torch.where(sqr_dist > 1e-7, torch.sqrt(sqr_dist + 1e-8), torch.zeros_like(sqr_dist)).topk(k, dim=1,
                                                                                                        largest=False)
    return dist, idx
